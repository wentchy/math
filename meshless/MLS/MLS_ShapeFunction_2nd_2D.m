function [phi, dphidx, dphidy] = MLS_ShapeFunction_2nd_2D(pt, node, di)
    
    n_basis = 6; % 1, x, y, x^2, y^2, xy --- 6 basis
    ...
    for I = 1 : m
        ...
        pI = [1; xI(1); xI(2); xI(1).^2; xI(2).^2; xI(1).*xI(2)];
        ppT = pI*pI';
        %ppT
        A = A + wi*ppT;
        %A
...
% $$$     dAdx
% $$$     dAdy
    p = [1; pt(1); pt(2); pt(1).^2; pt(2).^2; pt(1).*pt(2)];
    px = [0; 1; 0; 2*pt(1); 0; pt(2)];
    py = [0; 0; 1; 0; 2*pt(2); pt(1)];
