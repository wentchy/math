function [phi, dphidx, dphidy] = MLS_ShapeFunction_4th_2D(pt, node, di)
    
    n_basis = 15; % 1, x, y, x^2, y^2, xy, x^3, y^3, x^2*y,x*y^2
                  % x^4, x^3*y, x^2*y^2, x*y^3, y^4 --- 15 basis
    ...
    for I = 1 : m
        ...
        pI = [1; xI(1); xI(2); xI(1).^2; xI(2).^2; xI(1).*xI(2); xI(1).^3; ...
             xI(2).^3; xI(1).^2.*xI(2); xI(1).*xI(2).^2;xI(1).^4; ...
              xI(1).^3.*xI(2);xI(1).^2.*xI(2).^2;xI(1).* xI(2).^3; xI(2).^4];
        ppT = pI*pI';
        %ppT
        A = A + wi*ppT;
        %A
